...
- ✅ Phase 4 地圖探索：Leaflet+OSM圖磚（vendor複製自401，不重新下載）；地圖載入`spots`表，即時算每個熱點的機率分數並依分數變色（綠/橘/紅），點圖標彈窗顯示分數+訂閱按鈕（寫入`subscriptions`）。**`spots`表目前是空的**，`seed.sql`準備了6個知名地標當起始資料（阿里山祝山/合歡山主峰/武嶺/象山/貓空/龍磐草原，座標是常見公開地標概略值非逐點驗證），要看到地圖有圖標需要老闆先到SQL Editor跑一次
- ⏳ Phase 5-6 待做：即時回報動態牆+照片上傳+推播、品牌套用
