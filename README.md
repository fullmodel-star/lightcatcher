...
自然光影捕捉管家：黃金時刻/藍調時刻倒數、火燒雲指數、雲海指數＋雲海高度估算、觀星指數。山脈家族（k2, `#004F44` 玄冰青，2026-09-04 由赤陶紅 `#A64B38` 改版）第三支，PWA 架構仿 400 走稜步道（單檔 hash 路由）。
...
原始選圖 Tabler `sunset-2`（地平線+半圓落日），家族色 `#A64B38`。**2026-09-04 家族改版**：色改玄冰青 `#004F44`、圖案語彙也改成「一座山、一個字」系統（LIGHT＋Tabler `aperture`，見 `icon規格_v2.md` 指派表），本節文字未來得及跟著改，實際圖案以 App 端 `icon-512.png` 與品牌正式檔為準，未逐像素重新核對本段描述。已登記進 `05_品牌資源\03_App圖示_正式檔\02_安裝icon_其他家族\_README.md` 並產出正式檔（`icon-192.png`/`icon-512.png`），簽名帶檢測通過。
