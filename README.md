...
**這個指數只在資料庫明確標記過的地點顯示**(目前只有金龍山，`type='liuli_guang'`)，不像雲海/觀星可以套用在任何地點當附加參考——因為公式算不出「這個谷地下方到底有沒有城鎮燈光」這個地形前提，硬套會給出沒有意義的假分數。金龍山同時標記`sea_of_clouds`+`sunrise`+`liuli_guang`但**沒有標記`stars`**：同樣的城鎮燈光是光害，不適合觀星。已加入`seed.sql`(座標23.90023,120.90869、海拔820m查自日月潭觀光旅遊網官方頁面，**還沒跑進正式Supabase**，要重新執行一次`seed.sql`)、加入首頁精選景點快選、加入「今日推薦」排名(靠DB標記加入`TYPE_TO_KIND`，跟sea/glow/stars同一套邏輯)、目標時刻比照觀星抓真正入夜時段(同樣需要黑暗才看得到燈光透霧的效果)。
