...
**補查5個地點共6筆liuli_guang標記**（同日，老闆問「還有哪些地點會有琉璃光」後主動查證）：`隙頂二延平`/`頂石棹`/`鹿谷大崙山銀杏森林`(本來就在53個景點裡，只是額外補標記，座標/海拔沿用老闆原本資料)、`埔里虎頭山`(23.9739,120.9821, 555m)/`梅山太平雲梯`(23.559601,120.60008, 1000m)(全新景點)——都有新聞/官方頁面直接點名「琉璃光」，座標查自公開網路資料(非老闆提供)。全部6個地點都刻意沒標記`stars`：城鎮燈光是必要條件同時也是光害，兩者互斥。搜尋過程還看到「武界開心茶園」「二尖山」零星提及但證據不夠具體，先不加。**已寫進`seed.sql`但還沒跑進正式Supabase**，要重新執行一次`seed.sql`（54→58個景點，等冪安全可重跑）。
